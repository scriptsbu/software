...
while true;do
  ...
  case "${QUESTION}" in
    [Yy] ) 
      ...
      echo "alias vpn='sudo openconnect vpn.torcrobotics.com --authgroup=Employee-split-Push --user=$USER && resetdns'alias resetdns='sudo systemd-resolve --flush-caches'" | sudo tee -a ~/.bashrc
