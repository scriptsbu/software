...
function die {
    ...
    echo -e "$prefix: "
    echo -e "$prefix: ${bold}You can contact Vector for further assistance.${reset}"
    echo -e "$prefix: "
    
    echo -e "$prefix: ${bold}URL: https://vector.dev/community${reset}"
    echo -e "$prefix: "
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact Vector
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_gpg_key {
    local text="Importing 'timber/vector' repository GPG keys ..."
    echo_running "$text"
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/timber-vector.gpg
    }
